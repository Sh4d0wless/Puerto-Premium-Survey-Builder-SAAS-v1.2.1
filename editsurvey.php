						...
						while($q_rs = $q_sql->fetch_assoc()){
							...
							<div class='pt-new-question-content' rel='boxquestion_<?=$q_rs["sort"]?>' data-question='<?=$q_rs["sort"]?>' data-step='<?=$s_rs["sort"]?>'>
								...
								<div class='pt-new-answers-content' data-question='<?=$q_rs["sort"]?>' data-step='<?=$s_rs["sort"]?>'>
									<b><?=$lang['new']['new_a']?></b>
									<select class='pt-select' disabled>
										<option value='input' <?=($q_tp == '' ? 'input' : '')?> data-type='input'><?=$lang['new']['new_as1']?></option>
										<option value='textarea' <?=($q_tp == '' ? 'textarea' : '')?> data-type='text'><?=$lang['new']['new_as2']?></option>
										<option value='checkbox' <?=($q_tp == 'checkbox' ? 'selected' : '')?> data-type='checkbox'><?=$lang['new']['new_as3']?></option>
										<option value='radio' <?=($q_tp == 'radio' ? 'selected' : '')?> data-type='radio'><?=$lang['new']['new_as4']?></option>
										<option value='stars' <?=($q_tp == 'stars' ? 'selected' : '')?> data-type='stars'><?=$lang['new']['new_as5']?></option>
										<option value='date' <?=($q_tp == 'date' ? 'selected' : '')?> data-type='input'><?=$lang['new']['new_as6']?></option>
										<option value='phone' <?=($q_tp == 'phone' ? 'selected' : '')?> data-type='input'><?=$lang['new']['new_as7']?></option>
										<option value='country' <?=($q_tp == 'country' ? 'selected' : '')?> data-type='input'><?=$lang['new']['new_as8']?></option>
										<option value='email' <?=($q_tp == 'email' ? 'selected' : '')?> data-type='input'><?=$lang['new']['new_as9']?></option>
									</select>
									<div class="pt-radio-slide">
