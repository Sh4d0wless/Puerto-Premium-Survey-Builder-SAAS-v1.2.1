...
<table class="table">
	...
	<tbody>
		<?php
		$sql = $db->query("SELECT MAX(id) as rid, ip FROM ".prefix."responses WHERE survey = '{$id}' GROUP BY ip ORDER BY MAX(id) DESC LIMIT {$startpoint} , {$limit}") or die ($db->error);
		if($sql->num_rows):
		...
		<?php
		endwhile;
		echo (db_rows("responses WHERE survey = '{$id}' GROUP BY ip", "MAX(id)") > $limit ? '<tr><td colspan="12">'.fh_pagination("responses WHERE survey = '{$id}' GROUP BY ip",$limit, path."/responses.php?id={$id}&", 1, true).'</td></tr>' : '');
		else:
			?>
			<tr>
				<td colspan="12">
