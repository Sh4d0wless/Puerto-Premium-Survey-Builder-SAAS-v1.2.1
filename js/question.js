	...
	if(cnt > maxsteps){
		...
		ths.before(
			'<div class="pt-new-step-content" data-step="'+cnt+'">'+
				'<h4>Step ('+cnt+'): <a class="pt-deletestep-link" data-step="'+cnt+'">'+lang['new']['d_step']+'</a><a class="pt-add-question-link" data-step="'+cnt+'">'+lang['new']['new_q']+'</a></h4>'+
...
$(".pt-deletestep-link").livequery("click", function(){
	var ths  = $(this);
	var step = ths.data('step');

	$(".pt-new-step-content[data-step='"+step+"']").fadeOut(300, function(){ $(this).remove() });
	$(".pt-survey[data-step='"+step+"']").fadeOut(300, function(){ $(this).remove() });
...
$(".pt-add-answer-link").livequery("click", function(){
	...
	html += "<div class='boxanswer_"+anid+"' data-q='"+qts+"'>";
...
$('[class^="deleteboxanswer_"]').livequery("click", function(){
	var ths = $(this),
	     ...
	     ln = ths.parent().parent().find('[class^="deleteboxanswer_"]').length,
	     q  = ths.parent().data("q");
	if(confirm("Are you sure you want to delete this answer?")){
		...
		if($(".pt-editsurveypage").length){
			...
			if(ln == 1){
				$("[rel='boxquestion_"+q+"']").find("option").removeAttr('disabled');
			}
		}
	}
});
