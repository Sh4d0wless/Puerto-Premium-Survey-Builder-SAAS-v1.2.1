...
$("#sendnewsurvey").livequery("submit", function(){
	...
	$.post(path+"/ajax.php?pg=sendnewsurvey", ths.serialize(), function(puerto){
		if(puerto.type == 'success'){
			$.puerto_confirm("Success!", puerto.alert, "green");
			setTimeout(function () { $(location).attr('href', path+"/mysurvies.php"); }, 2000);
		} else {
			$.puerto_confirm("Error!", puerto.alert, "red");
		}
		console.log(puerto);
...
trigger.livequery('submit', function(){
 var $this  = $(this).find('button[clicked=true]'),
		 ...
		 if( step == 'end' ){
			 window.top.location.href = (url ? url : path+"/index.php");
		 } else {
...
//- Delete Page
$(".pt-delete-page").on('click', function() {
	if(confirm("Are you sure you want to delete this page?")){
		var pr = $(this).parent().parent().parent().parent();
		$.get(path+"/ajax.php?pg=delete&request=page&id="+ $(this).attr("rel"),function(puerto){
			console.log(puerto);
			pr.fadeOut();
		});
	}
...
$('.pt-exportall').on("click", function(e){
	...
	$.get(path+"/ajax.php?pg=exportall&id="+id, function(puerto) {
...
$.puertoAnswerImage = function(id, choose, thumb, thumb2, crop = true){
	...
	$(id).imageUploader({
	  ...
	  afterUpload: function (data) { console.log('after uploadee', data,name); name.val(data); $(thumb2).attr("src",data); },
	  onFileAdded: function(file)        { console.log(file); },
	  onFilesSelected: function()        { console.log('file selected'); },
	  onUrlSelected: function()          { console.log('url selected'); },
	  onDragStart: function(event)       { console.log(event); },
