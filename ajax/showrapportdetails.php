...
if($a_type == 'stars'){
	...
	while($rs_a = $sql_a->fetch_assoc()){
		...
		$sql_r = $db->query("SELECT MAX(survey) as msurvey, MAX(step) as mstep, MAX(question) as mquestion, ip FROM ".prefix."responses WHERE answer = '{$rs_a['aid']}' && response != '' GROUP BY ip ORDER BY MAX(id) ASC") or die ($db->error);
		while($rs_r = $sql_r->fetch_assoc()){
			$sql_rr = $db->query("SELECT * FROM ".prefix."responses WHERE survey = '{$rs_r['msurvey']}' && step = '{$rs_r['mstep']}' && question = '{$rs_r['mquestion']}' && ip = '{$rs_r['ip']}' && response != '' ORDER BY id ASC") or die ($db->error);
			$uu = '';
