...
<div class="pt-title">
	...
	<div class="pt-options">
		...
		<a href="#" class="pt-btn btn-green pt-exportall" rel="<?=$rs['id']?>"><i class="fas fa-file-csv"></i> <?=$lang['rapports']['btn3']?></a>
	</div>
</div>
...
<div class="row">
	<div class="col-6">
		...
		<div class="pt-div-stats">
			...
			<div><b><?=$lang['rapports']['responses']?></b> <?=db_rows("responses WHERE survey = '{$rs['id']}' GROUP BY ip ORDER BY MAX(id) DESC", "ip")?></div>
			<div>
...
while($s_rs = $s_sql->fetch_assoc()){
	...
	while($q_rs = $q_sql->fetch_assoc()){
		...
		echo "<div class='pt-rapport-q'>
						<div class='pt-answered'>{$lang['rapports']['by']} ".db_rows("responses WHERE question = '{$q_rs['id']}' && response != '' GROUP BY ip", "ip")." of {$s_rs['views']} {$lang['rapports']['people']}</div>
						<h3>".$q_rs['title']."
							<div class='pt-options'>
